...
export default function IssueReturnBooks() {
  ...
  async function issue() {
    setError("");
    setMessage("");
    if (!/^[0-9a-fA-F]{24}$/.test(form.userId.trim())) {
      setError("Enter a valid user id.");
      return;
    }
    try {
      await libraryService.issueBook(selected._id, {
  ...
  async function accept() {
    ...
    try {
      const res = await libraryService.returnBook(selected._id, {
        userId: form.userId.trim(),
      });
      ...
      <table className="data-table">
        ...
        <tbody>
          {books.map((b) => (
            <tr
              key={b._id}
              style={{ background: selected?._id === b._id ? "#eef2ff" : "" }}
            >
              <td>{b.title}</td>
              <td>{b.author}</td>
              <td>
